...
class MakeAuthorshipAuthorsPolymorphic < ActiveRecord::Migration[7.2]
  ...
  def up
    ...
    unless index_exists?(
      ...
      add_index :better_together_authorships, %i[author_type author_id],
                name: POLYMORPHIC_INDEX_NAME
    end
  end
  ...
  def down
    ...
    unless foreign_key_exists?(
      ...
      add_foreign_key :better_together_authorships, :better_together_people,
                      column: :author_id
    end

    remove_column :better_together_authorships, :author_type if column_exists?(:better_together_authorships,
                                                                               :author_type)
  end
end
