...
class AddFederationProvenanceToBetterTogetherPages < ActiveRecord::Migration[7.2]
  def change
    unless column_exists?(:better_together_pages, :platform_id)
      add_reference :better_together_pages, :platform, type: :uuid,
                                                       foreign_key: { to_table: :better_together_platforms }
    end
    add_column :better_together_pages, :source_id, :string unless column_exists?(:better_together_pages, :source_id)
    add_column :better_together_pages, :source_updated_at, :datetime unless column_exists?(:better_together_pages,
                                                                                           :source_updated_at)
    add_column :better_together_pages, :last_synced_at, :datetime unless column_exists?(:better_together_pages,
                                                                                        :last_synced_at)
