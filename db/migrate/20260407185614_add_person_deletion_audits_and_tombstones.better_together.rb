...
class AddPersonDeletionAuditsAndTombstones < ActiveRecord::Migration[7.1]
  def change
    add_column :better_together_people, :deleted_at, :datetime unless column_exists?(:better_together_people,
                                                                                     :deleted_at)
    add_column :better_together_people, :anonymized_at, :datetime unless column_exists?(:better_together_people,
                                                                                        :anonymized_at)

    unless table_exists?(:better_together_person_purge_audits)
      create_table :better_together_person_purge_audits,
                   id: :uuid
    end

    ensure_reference :better_together_person_purge_audits, :person, :better_together_people
    ensure_reference :better_together_person_purge_audits, :person_deletion_request,
    ...
    add_index :better_together_person_purge_audits, :status unless index_exists?(:better_together_person_purge_audits,
                                                                                 :status)
    add_index :better_together_person_purge_audits, :requested_at unless index_exists?(
      :better_together_person_purge_audits, :requested_at
    )
  end
