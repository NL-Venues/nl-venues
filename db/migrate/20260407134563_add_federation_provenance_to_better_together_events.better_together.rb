...
class AddFederationProvenanceToBetterTogetherEvents < ActiveRecord::Migration[7.2]
  def change
    unless column_exists?(:better_together_events, :platform_id)
      add_reference :better_together_events, :platform, type: :uuid,
                                                        foreign_key: { to_table: :better_together_platforms }
    end
    add_column :better_together_events, :source_id, :string unless column_exists?(:better_together_events, :source_id)
    add_column :better_together_events, :source_updated_at, :datetime unless column_exists?(:better_together_events,
                                                                                            :source_updated_at)
    add_column :better_together_events, :last_synced_at, :datetime unless column_exists?(:better_together_events,
                                                                                         :last_synced_at)
