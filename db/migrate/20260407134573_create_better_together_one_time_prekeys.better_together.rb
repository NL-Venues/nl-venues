...
class CreateBetterTogetherOneTimePrekeys < ActiveRecord::Migration[7.2]
  def change
    ...
    unless index_exists?(:better_together_one_time_prekeys,
                         %i[person_id
                            key_id])
      add_index :better_together_one_time_prekeys, %i[person_id key_id],
                unique: true
    end
    unless index_exists?(:better_together_one_time_prekeys,
                         %i[person_id consumed])
      add_index :better_together_one_time_prekeys,
                %i[person_id consumed]
    end
  end
end
