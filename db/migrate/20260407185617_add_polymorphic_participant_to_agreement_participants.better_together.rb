...
class AddPolymorphicParticipantToAgreementParticipants < ActiveRecord::Migration[7.2]
  ...
  def up
    ...
    unless column_exists?(:better_together_agreement_participants,
                          :participant_id)
      add_column :better_together_agreement_participants, :participant_id,
                 :uuid
    end
  ...
  def down
    ...
    if column_exists?(:better_together_agreement_participants,
                      :participant_type)
      remove_column :better_together_agreement_participants,
                    :participant_type
    end
    if column_exists?(:better_together_agreement_participants,
                      :participant_id)
