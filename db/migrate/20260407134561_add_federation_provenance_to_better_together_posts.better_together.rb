...
class AddFederationProvenanceToBetterTogetherPosts < ActiveRecord::Migration[7.2]
  def change
    unless column_exists?(:better_together_posts, :platform_id)
      add_reference :better_together_posts, :platform, type: :uuid,
                                                       foreign_key: { to_table: :better_together_platforms }
    end
    add_column :better_together_posts, :source_id, :string unless column_exists?(:better_together_posts, :source_id)
    add_column :better_together_posts, :source_updated_at, :datetime unless column_exists?(:better_together_posts,
                                                                                           :source_updated_at)
    add_column :better_together_posts, :last_synced_at, :datetime unless column_exists?(:better_together_posts,
                                                                                        :last_synced_at)
